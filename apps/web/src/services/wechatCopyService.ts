...
import { processHtml, createMarkdownParser } from "@paibanmiao/core";
...
const renderMermaidBlocks = async (container: HTMLElement): Promise<void> => {
  ...
  const renderIdBase = `paibanmiao-mermaid-${Date.now()}`;
  ...
  for (const [index, block] of mermaidBlocks.entries()) {
    ...
    try {
      ...
      console.error("[排版喵] Mermaid render failed:", error);
    }
  }
};
